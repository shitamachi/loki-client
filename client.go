...
const (
	...
	// ReservedLabelTenantID Label reserved to override the tenant ID while processing
	// pipeline stages
...
var UserAgent = fmt.Sprintf("promtail/version")
...
func (c *client) sendBatch(tenantID string, batch *batch) {
	...
	backoff := NewBackoff(c.ctx, c.cfg.BackoffConfig)
	var status int
	for {
		start := time.Now()
				...
				for i := range lbls {
					if lbls[i].Name == LatencyLabel {
						lblSet = model.LabelSet{
							HostLabel:    model.LabelValue(c.cfg.URL.Host),
							LatencyLabel: model.LabelValue(lbls[i].Value),
						}
					}
				}
				if lblSet != nil {
